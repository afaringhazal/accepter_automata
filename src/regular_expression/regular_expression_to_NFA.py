...
def create_finite_automaton(et):
...
def evalRegexConcat(et):
    left_nfa = create_finite_automaton(et.left)
    right_nfa = create_finite_automaton(et.right)
...
def evalRegexUnion(et):
    ...
    up_nfa = create_finite_automaton(et.left)
    down_nfa = create_finite_automaton(et.right)
...
def evalRegexKleene(et):
    ...
    sub_nfa = create_finite_automaton(et.left)
...
def printTransitionTable(finite_automata):
    ...
    f.write("State\t\tSymbol\t\t\tNext state\n")
    printStateTransitions(finite_automata[0], [], {finite_automata[0]: 0})

def read_file():
    with open("RE_Input_3.txt") as fp:
        Lines = fp.readlines()
        count = 1
        alphabet: List[str] = []
        for line in Lines:
            ...
            elif count == 2:  # regular expression
                regular_expression = line.strip()
            count += 1
        return alphabet, regular_expression

alphabet, regular_expression = read_file()
pr = postfix(regular_expression)
et = constructTree(pr)

# inorder(et)
f = open("NFA_Output_2.txt", "w")
finite_automata = create_finite_automaton(et)
printTransitionTable(finite_automata)
f.close()